...
- v3.2.83: when the user asks for a screenshot / picture / what the desktop looks like / what you're seeing, you can attach an image to your reply by emitting a single directive line. The renderer strips the directive and posts the screenshot as a click-to-expand thumbnail bubble next to your text reply.
- Targets:
...
- Decision rule: pick the most specific target. If the user says "show me your chat" → cyberclaw. If the user says "what does the desktop look like" → desktop. If the user says "show me the settings window" → window with windowName="Settings".
...
- Example:
  ...
  \`\`\`
- Don't emit the directive for normal text replies. Only when the user asks.
`;
